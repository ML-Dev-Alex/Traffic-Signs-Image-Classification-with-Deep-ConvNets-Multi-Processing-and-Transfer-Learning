...
if __name__ == '__main__':
    ...
    train_images_per_class = 32
    val_images_per_class = 4
    variations = 4
    ...
    # We will use transfer learning from the Xception model.
    model = Xception(weights='imagenet', include_top='false', pooling='max', input_shape=(image_size, image_size, 3))
    ...
    with open('Xception_color_architecture.json', 'w') as f:
    ...
    for i in range(batches):
        ...
        X_train = np.zeros((number_of_classes * train_images_per_class * variations, image_size, image_size, 3),
                           dtype=np.float32)
        y_train = []

        X_val = np.zeros((number_of_classes * val_images_per_class * variations, image_size, image_size, 3),
                         dtype=np.float32)
        y_val = []

        # Populate class vectors with [images_per_class * variations, image_size, image_size, 3] images,
        # We then concatenate these vectors into a single vector with the data from all classes.
        X_train, y_train, class_indexes = helpers.populate_vector_color(x_array=X_train, y_list=y_train,
                                                                        folder_path=folder,
        ...
        X_val, y_val, class_indexes = helpers.populate_vector_color(x_array=X_val, y_list=y_val,
                                                                    folder_path=folder,
                                                                    class_dataframes=class_dataframes,
                                                                    number_of_classes=number_of_classes,
